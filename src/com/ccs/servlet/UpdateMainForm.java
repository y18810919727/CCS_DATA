...
@WebServlet(name = "UpdateMainForm", urlPatterns = { "/doUpdateMain.servlet" })
public class UpdateMainForm extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 将数据封装进production对象中，同步修改production中的目标值
		...
		// // 取得单号最后两位数字的整型形式并加1
		// int num = Integer.parseInt(curnumber.substring(8)) + 1;
		// // 取得单号的日期部分
		// String nextnumber = curnumber.substring(0, 8);
		// // 生成新单号
		// if (num < 10)
		// nextnumber = nextnumber + "0" + num;
		// else
		// nextnumber = nextnumber + num;
		// 获取当前单号在summary表中的id
		// 注意：请务必保持ID连续！！
		int sumid = Integer.parseInt(request.getParameter("sumid"));
		try {
			boolean stockres = new StockDAO().UpdateOne(stock);
			boolean summaryres = new SummaryDAO().UpdateOne(summary);
			// 修改下一个单号中的“换料前”数据(按照数据库id搜寻)
			boolean updatenext = new SummaryDAO().UpdatePreData(summary,
					sumid + 1);
			boolean productions = new ProductionDAO().UpdateTargetByNum(p);
			boolean ingredients = new IngredientDAO().UpdateOne(ingredient);
			/*
			 * System.out.println("stock：" + stockres);
			 * System.out.println("summary：" + summaryres);
			 * System.out.println("NextNumPreData：" + updatenext);
			 * System.out.println("ingredient：" + ingredients);
			 * System.out.println("production：" + productions);
			 */
		...
		if (isSuccess) {
			System.out
					.println("用户修改配料单成功，单号：" + request.getParameter("number"));
			PrintWriter out = response.getWriter();
			out.println("<meta http-equiv='refresh' content='2; url=/CCS/mainform.jsp'> ");
			out.println("数据更新成功...<br>如果您的浏览器没有自动跳转，请"
			...
			PrintWriter out = response.getWriter();
			out.println("<meta http-equiv='refresh' content='2; url=/CCS/mainform.jsp'> ");
			out.println("数据更新失败！即将返回到查询页面...<br>如果您的浏览器没有自动跳转，请"
					+ "<a href='/CCS/mainform.jsp'>点击这里返回</a>");
			out.flush();
			out.close();
		}
